import { PlayerStat } from '@/components/pvp/PvPScoreboard';

/** Mock-only extension: trend fields aren't part of the live scoreboard schema yet. */
type MockPlayerStat = PlayerStat & { trend: 'up' | 'down' | 'stable'; trendValue: number };
...
export function generateMockPlayers(count: number = 30): MockPlayerStat[] {
  const players: MockPlayerStat[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    players.push({
      ...
      headshots: headshots,
      playtime: playtime,
      longestShot: Math.floor(Math.random() * 900) + 50,
