...
function confidenceClass(confidence: 'low' | 'medium' | 'high') {
  ...
  if (confidence === 'medium') return 'text-amber-300 border-amber-500/30 bg-amber-950/20';
    return 'text-gray-600 dark:text-neutral-300 border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-neutral-900/60';
}
...
export function ForecastPanel({ analytics }: ForecastPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-white/5 bg-white/70 dark:bg-neutral-900/40 backdrop-blur-sm p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          {forecast.map((point) => (
            <div
              ...
              className="rounded-lg border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-black/20 p-2.5 flex flex-col gap-1"
            >
              <p className="text-xs text-gray-400 dark:text-neutral-500">+{point.hourOffset}h</p>
              <p className="text-sm text-gray-600 dark:text-neutral-200 font-medium">{point.label}</p>
              <p className="text-lg leading-none text-gray-900 dark:text-white font-bold">{point.predictedPlayers}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {anomalySummary && (
        <p className="text-sm text-amber-600 dark:text-amber-300/90 border-t border-gray-200 dark:border-white/5 pt-3">{anomalySummary}</p>
      )}
    </div>
  );
}
